...
final class GetPersonCrewUseCase: ClosureUseCase {

    typealias RequestType = Int
    typealias ResultType = [UiCrew]
    ...
    func execute(
        _ parameters: Int,
        _ onCompletion: @escaping (ApiResult<[UiCrew]>
        ) -> Void) {
        repository.getPersonCrew(id: parameters) { [mapper] in
